  ...
  <style>
    ...
    @media (max-width: 480px) {
      ...
      .wave-title span {
        ...
        font-size:  clamp(3.8rem, 3vw + 1rem, 6rem);
        color: #ff9000;
        text-shadow: 2px 2px #ffcc00;
        letter-spacing: 4px;
      }
...
<div class="section yellow-bg py-5 position-relative" style="min-height:100vh;overflow:hidden;">
  ...
  <div class="rellax" data-rellax-speed="-2" style="
    background-image: url('images/wave-other.svg');
    background-size: cover;
    background-position: center;
    position: absolute;
  ...
  <div class="container" id="fun-facts"  data-aos="zoom-out-down" style="position: relative;z-index: 2;">
    ...
    <div class="row justify-content-center" style="z-index: 999;">
      ...
      @php
        $positionInRow = $index % 3; // posisi dalam 1 baris (0,1,2)
        $rowNumber = intdiv($index, 3); // baris keberapa (0,1,2)

        // Default rotasi baris biasa
        $rotations = [-10, 0, 10];
        
        // Khusus untuk baris terakhir kalau card sisa
        $remainingCards = count($funfacts) % 3;
        $isLastRow = $index >= count($funfacts) - $remainingCards;

        if ($isLastRow && $remainingCards !== 0) {
            if ($remainingCards == 1) {
                $rotation = 0;
            } elseif ($remainingCards == 2) {
                $rotation = ($positionInRow == 0) ? -10 : 10;
            }
        } else {
            // Untuk baris normal
            if ($rowNumber % 2 == 0) {
                ...
                $rotation = $rotations[2 - $positionInRow];
            }
        }
      @endphp
