...
class LibraryController extends Controller
{
    ...
    public function actionLoginLibrary(Request $request)
    {
        try {
            ...
            if($checkRole == 4) {
                $response = Http::timeout(5)->get("http://127.0.0.1:8080/api/get-detail-students/{$user->id}");
                session()->put([
                    'id_user' => $user->id,
                    'role' => "student",
            ...
            elseif($checkRole == 6){
                session()->put(['role' => "library"]);
                return redirect('/library/gcs/section/admin');
            }
        } catch (Exception $err) {
            return dd($err);
        }
    ...
    public function library(){
        ...
        ->orderBy('rack_location', 'ASC')->paginate(5);
    ...
    public function updateThreeLevel(Request $request){
        if(count($request->files) == 0){
            ...
            $filePath = null;
            $cupboardThreeLevel = Cupboard_three_level::findOrFail($request->id);
    ...
    public function reserveBook(Request $request){
        $checkAvailable = Reserve_book::where('user_id', $request->user_id)
        ->where('book_id', $request->book_id)
        ->where('status', 0)
        ->get();

        if(count($checkAvailable) != 0){
            session()->flash('already_exists');
            return redirect()->back();
        }
        else{
            Reserve_book::create([
    ...
    public function reserve()
    {
        ...
        $data->getCollection()->transform(function ($item) use ($studentsById) {
            $item->student = $studentsById[$item->user_id] ?? null;
            return $item;
        });
    ...
    public function facility(){
        ...
        return view('components.library.facility');
    }
